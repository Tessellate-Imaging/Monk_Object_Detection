...
class Infer():
    ...
    def Model(self, cfg_path, class_list, weight, use_gpu=True, input_size=416, half_precision=False):
        '''
        ...
        Returns:
            None
        '''
        self.system_dict["params"]["cfg"] = cfg_path; 
        ...
        self.system_dict["local"]["device"] = torch_utils.select_device(False if use_gpu else True);
    ...
    def Predict(self, img_path, conf_thres=0.3, iou_thres=0.5):
        ...
        for path, img, im0s, vid_cap in self.system_dict["local"]["dataset"]:
            ...
            # Apply NMS
            pred = non_max_suppression_new(pred, self.system_dict["params"]["conf_thres"], 
    ...
    def Validate(self, img_dir, label_dir, classes, batch_size=16, img_size=416, conf_thres=0.001, iou_thres=0.5, nms_thres=0.5):
    	with torch.no_grad():
    		results = validate(self.system_dict["params"]["cfg"],
														         img_dir,
														         label_dir,
														         classes,
														         weights=None,
