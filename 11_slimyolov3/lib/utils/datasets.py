...
class LoadImagesAndLabels(Dataset):  # for training/testing
    def __init__(self, img_dir, label_dir, img_size=416, batch_size=16, augment=False, hyp=None, rect=True, image_weights=False,
                 cache_images=False):
        '''
        path = str(Path(path))  # os-agnostic
        ...
        assert os.path.isdir(label_dir), 'Label Dir not found %s. See %s' % (label_dir, help_url)
        ...
        assert n > 0, 'No images found in %s' % img_dir
