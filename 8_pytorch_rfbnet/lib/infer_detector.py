...
class Infer():
    ...
    def Pretrained_Model(self, model_name="vgg", use_gpu=True):
        '''
        User function: Selet trained model params
        ...
        if(model_name == "e_vgg"):
            self.system_dict["params"]["version"] = "RFB_E_vgg";
            self.system_dict["params"]["trained_model"] = "RFB512_E_34_4.pth"
            self.system_dict["params"]["size"] = 512;
        elif(model_name == "mobilenet"):
            self.system_dict["params"]["version"] = "RFB_mobile";
            self.system_dict["params"]["trained_model"] = "RFB_mobile_20_7.pth"
            self.system_dict["params"]["size"] = 300;
        ...
        self.system_dict["local"]["classes"] = ["background",
                                                        "person",
                                                        "bicycle",
                                                        "car",
                                                        "motorcycle",
    ...
    def Predict_On_Image(self, img_file, output_img_path="output.png", thresh=0.7, font_size=1, line_size=5):
        '''
        User function: Run inference on image and visualize it
    ...
    def Predict_On_Folder(self, folder_path,  thresh=0.7, font_size=1, line_size=5, output_folder_path="output_folder"):
        ...
        for i in tqdm(range(len(img_list))):
            ...
            scores, bboxes, labels = self.Predict_On_Image(input_image, thresh=thresh, font_size=font_size, line_size=line_size, output_img_path=output_image);
    ...
    def Evaluate_On_Folder(self, image_folder, gt_folder, output_folder_path='eval_result', thresh=0.3, iou_thresh=0.5, font_size=1, line_size=3):
        img_list = sorted(os.listdir(image_folder));
        anno_list = sorted(os.listdir(gt_folder));

        output = self.Predict_On_Folder(image_folder, thresh=0.7, font_size=1, line_size=5, output_folder_path=output_folder_path);
